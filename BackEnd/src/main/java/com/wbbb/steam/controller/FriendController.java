...
public class FriendController {
    ...
    /** 获取好友列表 */
    @GetMapping("/list")
    public ResponseDto<List<UserSimpleDto>> getFriendList(@RequestHeader("token") String token) {
    ...
    /** 获取已发送的好友申请 */
    @GetMapping("/invite")
    public ResponseDto<List<UserSimpleDto>> getSentFriendInvitations(@RequestHeader("token") String token) {
        Long userId = userService.parseToken(token);
        if (userId == null)
            return ResponseDto.unauthorized(null, "登录状态已失效，请重新登录");
    ...
    /** 获取未处理的好友申请 */
    @GetMapping("/pending")
    public ResponseDto<List<UserSimpleDto>> getPendingFriendInvitations(@RequestHeader("token") String token) {
        Long userId = userService.parseToken(token);
        if (userId == null)
            return ResponseDto.unauthorized(null, "登录状态已失效，请重新登录");
